...
export default function DocxTableDocument({ sourceUrl, onSelect, fullWidth = true, headingOverride }) {
  ...
  const renderParagraph = (paragraph) => <p key={paragraph.id} style={paragraph.style} className="min-h-[14px] whitespace-pre-wrap font-document leading-normal">{paragraph.runs.map((run, index) => <span key={run.id} style={run.style}>{headingOverride && index === 0 && run.text.includes("B1 Destination Unit") ? headingOverride : run.text}</span>)}</p>;
