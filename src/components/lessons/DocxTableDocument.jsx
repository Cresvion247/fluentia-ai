...
export default function DocxTableDocument({ sourceUrl, onSelect, fullWidth = true, headingOverride, highlightWords = [], showParagraphs = true, getSectionHeading }) {
  ...
  const renderParagraph = (paragraph, index, paragraphs) => {
    ...
    const sectionMatch = getSectionHeading && paragraphText.trim().match(/^(\d{1,2})(?=$|[A-Z])/);
    const isStandaloneNumber = sectionMatch && paragraphText.trim() === sectionMatch[1];
    const nextText = paragraphs[index + 1]?.runs.map((run) => run.text).join("") || "";
    const sectionContent = isStandaloneNumber ? nextText : paragraphText.slice(sectionMatch?.[1].length || 0);
    const sectionTitle = sectionMatch && getSectionHeading(sectionMatch[1], sectionContent);
    return <React.Fragment key={paragraph.id}>{sectionTitle && <h2 className="mb-2 mt-3 font-heading text-base font-bold text-indigo-700">{sectionMatch[1]} · {sectionTitle}</h2>}{!isStandaloneNumber && <p style={paragraph.style} className="min-h-[14px] whitespace-pre-wrap font-document leading-normal">{isOverriddenHeading ? <span style={paragraph.runs[0]?.style}>{headingOverride}</span> : paragraph.runs.map((run, runIndex) => <span key={run.id} style={run.style}>{highlightText(runIndex === 0 && sectionMatch ? run.text.replace(sectionMatch[1], "") : run.text)}</span>)}</p>}</React.Fragment>;
  };
