...
export default function TextSelectionMenu() {
  ...
  if (!menu) return null;
  const position = { left: Math.min(menu.x, window.innerWidth - 280), top: Math.min(menu.y, window.innerHeight - 180) };
  return <div onClick={(event) => event.stopPropagation()} style={position} className="fixed z-50 w-64 rounded-xl border border-slate-200 bg-white p-2 shadow-xl dark:border-white/15 dark:bg-slate-900">
    ...
    {menu.levels && <div className="flex gap-1 px-2 pb-2"><button onClick={() => ask(`Paraphrase this English text at A2 level. Return only the paraphrase: ${menu.text}`)} className="rounded bg-slate-100 px-2 py-1 text-xs dark:bg-white/10">A2</button><button onClick={() => ask(`Paraphrase this English text at B1 level. Return only the paraphrase: ${menu.text}`)} className="rounded bg-slate-100 px-2 py-1 text-xs dark:bg-white/10">B1</button><button onClick={() => ask(`Paraphrase this English text at C1 level. Return only the paraphrase: ${menu.text}`)} className="rounded bg-slate-100 px-2 py-1 text-xs dark:bg-white/10">C1</button></div>}
