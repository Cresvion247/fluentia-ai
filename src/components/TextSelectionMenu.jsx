...
import PronunciationDiagramModal from "@/components/PronunciationDiagramModal";

export default function TextSelectionMenu() {
  const [menu, setMenu] = useState(null);
  const [result, setResult] = useState("");
  const [loading, setLoading] = useState(false);
  const [guideGroup, setGuideGroup] = useState(null);
  ...
  useEffect(() => {
    const openMenu = (event) => {
      ...
      setResult("");
      const phonemeGroup = event.target.closest?.("[data-phoneme-group]")?.dataset.phonemeGroup;
      setMenu({ text, x: event.clientX, y: event.clientY, levels: false, phonemeGroup });
    };
  ...
  return <div onClick={(event) => event.stopPropagation()} style={position} className="fixed z-50 max-h-72 w-64 overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-xl dark:border-white/15 dark:bg-slate-900">
    ...
    {menu.phonemeGroup && <button onClick={() => setGuideGroup(menu.phonemeGroup)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-white/10"><Volume2 size={16}/>Pronunciar</button>}
    ...
    <PronunciationDiagramModal group={guideGroup} onClose={() => setGuideGroup(null)}/>
  </div>;
}